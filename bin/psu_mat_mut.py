...
def mutate(data_path,
    ...
    if 'HV' in bin_dimensions:
        ...
        for i in range(bins):
            for j in range(len(HV_labels)):
                if HV_values[j] >= edges_HV[
                        i] and HV_values[j] < edges_HV[i + 1]:
                    counter += 1
                    HVID = HV_labels[j]
    ...
    if 'SA' in bin_dimensions:
        ...
        for i in range(bins):
            for j in range(len(SA_labels)):
                if SA_values[j] >= edges_SA[
                        i] and SA_values[j] < edges_SA[i + 1]:
                    SAID = SA_labels[j]
    ...
    if 'CH4' in bin_dimensions:
        ...
        for i in range(bins):
            for j in range(len(CH4_labels)):
                if CH4_values[j] >= edges_CH4[
                        i] and CH4_values[j] < edges_CH4[i + 1]:
                    CH4ID = CH4_labels[j]
    ...
    top_path = str(atom_types) + 'atmtyp_' + str(number_of_children) + \
        'mutants_strength' + str(int(strength * 100))
...
# n1, n2, x_o, y_o, z_o, q_o = np.genfromtxt(mat_path +       ## change
# this for next gens
        n1, n2, x_o, y_o, z_o = np.genfromtxt(data_path + '/' + parent_ID +
                                              '/' + parent_ID + '.cif',
                                              unpack=True,
                                              # skip_header=19)  ## change this
                                              # for next gens
                                              skip_header=17)
