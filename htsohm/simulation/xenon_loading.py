...
def write_raspa_file(filename, run_id, material_id, helium_void_fraction ):
    """Writes RASPA input file for simulating XENON loading.
...
def run(run_id, material_id, helium_void_fraction):
    ...
    Returns:
        results (dict): xenon loading simulation results.
    ...
    filename = os.path.join(output_dir, "XenonLoading.input")
    write_raspa_file(filename, run_id, material_id, helium_void_fraction)
    while True:
        try:
            print("Date :\t%s" % datetime.now().date().isoformat())
            print("Time :\t%s" % datetime.now().time().isoformat())
            print("Simulating xenon loading in %s-%s..." % (run_id, material_id))
            subprocess.run(['simulate', './XenonLoading.input'], check=True, cwd=output_dir)
