...
def parse_output(output_dir, simulation_config):
    ...
    for p in [ga0_pressure, ga1_pressure]:
        if p != None:
            ...
            f = 'ga01'  # flag for second pressure gas adsorption simulations

    # calculate bin
    if isinstance(external_pressure, list):
        value = abs(results['ga00_absolute_volumetric_loading'] - results['ga01_absolute_volumetric_loading'])
    else:
        value = results['ga00_absolute_volumetric_loading']
