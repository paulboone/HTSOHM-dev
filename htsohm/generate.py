...
def generate(number_of_materials, atom_types, run_id, 
    max_number_density=0.084086, min_number_density=0.000013907, max_abc=52.392,
    min_abc=13.098, max_epsilon=513.264, min_epsilon=1.258, max_sigma=6.549,
    ...
    for i in range(number_of_materials):         # each iteration creates a new material
        ...
        os.mkdir( def_dir )
        cif_file = os.path.join(mat_dir, mat_name + ".cif")              # structure file
        ...
        for i in range(atom_types):
            epsilon = round(random() * (max_epsilon - min_epsilon) + 
                min_epsilon, 4)
            ep.append(epsilon)
        ...
        if len(a_count) != 1:            # more than one atom type!
            temp_lcm = lcmm(*a_count)    # maxiumum charge multiplier
            cm_max = floor(max_charge / (temp_lcm * elem_charge / min(a_count)))
            cm_list = []
            ac_len = len(a_count)

            for i in range(ac_len - 1):  # choose 1st n-1 charge multipliers
        ...
        for i in range(atom_types):      # write mixing_rules and pseudo_atoms
            with open(psu_file, "a") as file:
                file.write(
                    "A_%s\tyes\tC\tC\t0\t12.\t" % (int(atoms[i,0])) +
                    "%s\t0\t0\t1\t1\t0\t0\t" % (atoms[i,3]) +
                    "absolute\t0\n")

            with open(mix_file, "a") as file:
                file.write(
                    "A_%s\tlennard-jones\t" % (int(atoms[i,0])) +
                    "%s\t%s\n" % (atoms[i,1], atoms[i,2]))

        for i in range(n_):              # write .cif-file
            x = round(random(), 4)
            y = round(random(), 4)
            z = round(random(), 4)
...
# H2       :   H_h2; H_com
        ...
        with open(psu_file, "a") as file:
            file.write(
                "N_n2\tyes\tN\tN\t0\t14.00674\t-0.4048\t0.0\t1.0\t" +
                "0.7\t0\t0\trelative\t0\n" +
